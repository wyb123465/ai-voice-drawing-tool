...
- 支持常见中文别名，如“圆形/圆圈/圆”“矩形/长方形/方块”。
- 支持中文数量词，如“两座山”。
- 对复合句按“然后、接着、再、并且、逗号”等切分。
- 相对位置指令会寻找最近匹配的锚点图形。
- 未识别指令不会改变画布，只给出语音反馈和日志。

## 响应延迟

本版的指令理解和绘图执行都在浏览器本地完成，不需要等待云端模型。语音识别完成后，解析和状态更新通常是毫秒级，主要延迟来自浏览器语音识别本身。
...
| 多对象精确选择 | 语音中的“第二个蓝色圆”需要更完整的对象索引语言 | 已支持 first/last 方向选择与颜色筛选（如“删除红色的圆”“把蓝色的圆改成红色”），后续增加对象编号与命名反馈 |
| 多轮澄清 | 当前未识别时只反馈失败，没有继续追问 | 引入澄清状态机 |
