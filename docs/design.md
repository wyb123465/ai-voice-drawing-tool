...
- 相对位置指令会寻找最近匹配的锚点图形。
- 编辑指令支持 first/last、焦点指代、颜色筛选和第 N 个匹配对象，降低同类对象较多时误操作的概率。
...
| 对象命名与澄清选择 | 当前已支持 first/last、焦点指代、颜色筛选和“第 N 个匹配对象”，但还不能给对象起名或在歧义时追问确认 | 后续增加对象命名反馈，如“把这个叫小太阳”，并在歧义时进入澄清状态 |
