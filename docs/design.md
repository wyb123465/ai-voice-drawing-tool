...
- 语音识别循环会区分“用户希望继续聆听”和“浏览器当前连接状态”；句子结束、静默或可恢复错误后自动重连，只有用户点击停止或权限类错误才退出语音模式。
- 对低置信移动表达做一轮澄清追问，如“整个三角形上去”“把它上去”和“把这个圆上去”会先确认是否要向上移动目标；如果点名形状当前不在画布上，则阻断澄清、缩放和换色，避免确认或执行一个无法完成的动作。
